...
func GenerateSchoolYear(year uint32) string {
...
// NullStringScan function to convert string pointer to string
// Params:
...
func StructToByte(data interface{}) ([]byte, *constants.ErrorResponse) {
...
func TimeFormatToHourMinute(s string) (int, int, *constants.ErrorResponse) {
	...
	if err != nil {
		return 0, 0, constants.Error(http.StatusInternalServerError, codes.Internal, constants.DefaultCustomErrorCode, err.Error())
	}
...
func QueryLogNamed(query string, input interface{}) {
...
func QueryLogMysql(query string, args ...interface{}) {
	query = strings.ReplaceAll(query, "?", "%v")
	fmt.Println(fmt.Sprintf(query, args...))
}
...
func GenerateRedisKey(appName, purpose, id, device, uniqueKey string, allowMultipleLogin bool) string {
	var result string
	if allowMultipleLogin {
		result = fmt.Sprintf("%s-%s-%s:%s-%s", appName, purpose, device, id, uniqueKey)
	} else {
		result = fmt.Sprintf("%s-%s-%s:%s", appName, purpose, device, id)
	}
	return result
}
...
func StringToTime(s string) (time.Time, *constants.ErrorResponse) {
	...
	t, err := time.Parse(constants.DateRFC, s)
	if err != nil {
		return t, constants.ErrorInternalServer(err.Error())
	}
	return t, nil
}
...
func StringToDate(s string) (time.Time, *constants.ErrorResponse) {
	...
	t, err := time.Parse(constants.DateFormatStd, s)
	if err != nil {
		return t, constants.ErrorInternalServer(err.Error())
	}
	return t, nil
