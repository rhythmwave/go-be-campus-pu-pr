...
type lecturerResignationHandler struct {
	*service.ServiceCtx
}

func (l lecturerResignationHandler) GetList(w http.ResponseWriter, r *http.Request) {
	...
	defer l.AdminActivityLogService.Create(ctx, r, time.Now(), "Lecturer Resignation", "Get List", nil)
	...
	data, errs := l.LecturerResignationService.GetList(ctx, paginationData, in.GetStudyProgramId(), in.GetIdNationalLecturer(), in.GetSemesterId())
	if errs != nil {
	...
	for _, v := range data.Data {
		resultData = append(resultData, &GetListResponseData{
			...
			ResignDate:            v.ResignDate.Format(constants.DateRFC),
	...
	result = GetListResponse{
		Meta: &Meta{
			Message: "Get List Lecturer Resignation",
...
func (l lecturerResignationHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	defer l.AdminActivityLogService.Create(ctx, r, time.Now(), "Lecturer Resignation", "Create", &in)

	resignDate, errs := utils.StringToTime(in.GetResignDate())
	if errs != nil {
		utils.PrintError(*errs)
		result = CreateResponse{
			Meta: &Meta{
	...
	data := objects.CreateLecturerResignation{
		LecturerId:        in.GetLecturerId(),
		SemesterId:        in.GetSemesterId(),
		ResignDate:        resignDate,
		ResignationNumber: in.GetResignationNumber(),
		Purpose:           in.GetPurpose(),
		Remarks:           in.GetRemarks(),
	}
	errs = l.LecturerResignationService.Create(ctx, data)
	if errs != nil {
		utils.PrintError(*errs)
		result = CreateResponse{
			Meta: &Meta{
	...
	result = CreateResponse{
		Meta: &Meta{
			Message: "Create Lecturer Resignation",
			Status:  http.StatusOK,
			Code:    constants.SuccessCode,
		},
	}
