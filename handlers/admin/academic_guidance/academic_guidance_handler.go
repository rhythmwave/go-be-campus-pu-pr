...
func (f academicGuidanceHandler) GetListStudent(w http.ResponseWriter, r *http.Request) {
	...
	data, errs := f.AcademicGuidanceService.GetListStudent(ctx, paginationData, in.GetLecturerId(), in.GetSemesterId())
	if errs != nil {
...
func (f academicGuidanceHandler) Upsert(w http.ResponseWriter, r *http.Request) {
	...
	defer f.AdminActivityLogService.Create(ctx, r, time.Now(), "Academic Guidance", "Upsert", &in)

	data := objects.UpsertAcademicGuidance{
		SemesterId: in.GetSemesterId(),
		LecturerId: in.GetLecturerId(),
		StudentIds: in.GetStudentIds(),
	}
	errs := f.AcademicGuidanceService.Upsert(ctx, data)
	if errs != nil {
		utils.PrintError(*errs)
		result = UpsertResponse{
			Meta: &Meta{
	...
	result = UpsertResponse{
		Meta: &Meta{
			Message: "Upsert Academic Guidance",
			Status:  http.StatusOK,
			Code:    constants.SuccessCode,
		},
	}
...
func (f academicGuidanceHandler) UpsertDecision(w http.ResponseWriter, r *http.Request) {
	var result UpsertDecisionResponse

	ctx := r.Context()
	var in UpsertDecisionRequest
	err := json.NewDecoder(r.Body).Decode(&in)
	if err != nil {
		logrus.Errorln(err)
		result = UpsertDecisionResponse{
			Meta: &Meta{
				Message: err.Error(),
				Status:  http.StatusInternalServerError,
				Code:    constants.DefaultCustomErrorCode,
	...
	defer f.AdminActivityLogService.Create(ctx, r, time.Now(), "Academic Guidance", "Upsert Decision", &in)

	decisionDate, errs := utils.StringToTime(in.GetDecisionDate())
	if errs != nil {
		utils.PrintError(*errs)
		result = UpsertDecisionResponse{
			Meta: &Meta{
	...
	data := objects.UpsertDecisionAcademicGuidance{
		LecturerId:     in.GetLecturerId(),
		SemesterId:     in.GetSemesterId(),
		DecisionNumber: in.GetDecisionNumber(),
		DecisionDate:   decisionDate,
	}
	errs = f.AcademicGuidanceService.UpsertDecision(ctx, data)
	if errs != nil {
		utils.PrintError(*errs)
		result = UpsertDecisionResponse{
			Meta: &Meta{
	...
	result = UpsertDecisionResponse{
		Meta: &Meta{
			Message: "Upsert Decision Academic Guidance",
			Status:  http.StatusOK,
			Code:    constants.SuccessCode,
		},
	}
...
func (f academicGuidanceHandler) CreateSession(w http.ResponseWriter, r *http.Request) {
	...
	data := objects.CreateAcademicGuidanceSession{
		AcademicGuidanceId: in.GetAcademicGuidanceId(),
