...
func (f classExamHandler) GetList(w http.ResponseWriter, r *http.Request) {
	...
	data, errs := f.ClassExamService.GetList(ctx, paginationData, appConstants.AppTypeStudent, nil)
	if errs != nil {
...
func (c classExamHandler) Submit(w http.ResponseWriter, r *http.Request) {
	...
	defer c.LecturerStudentActivityLogService.Create(ctx, r, time.Now(), "Class Exam", "Submit", nil)

	errs := c.ClassExamService.Submit(ctx, in.GetClassExamId(), in.GetFilePath(), in.GetFilePathType())
	if errs != nil {
		utils.PrintError(*errs)
		result = SubmitResponse{
			Meta: &Meta{
