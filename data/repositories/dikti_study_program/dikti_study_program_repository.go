...
type diktiStudyProgramRepository struct {
	*db.DB
}
...
func mapQueryFilterGetList(search string, params *[]interface{}) string {
	...
	if search != "" {
		filterArray = append(filterArray, "dsp.name ILIKE $%d")
...
func (d diktiStudyProgramRepository) GetList(ctx context.Context, tx *sqlx.Tx, pagination common.PaginationRequest) ([]models.GetDiktiStudyProgramList, common.Pagination, *constants.ErrorResponse) {
	resultData := []models.GetDiktiStudyProgramList{}
	...
	if err := utils.QueryOperation(&queryGet, map[string]string{"dsp.name": constants.Ascending}, "", uint32(pagination.Limit), uint32(pagination.Page)); err != nil {
