...
type expertiseGroupRepository struct {
	*db.DB
}
...
func (f expertiseGroupRepository) GetList(ctx context.Context, tx *sqlx.Tx, pagination common.PaginationRequest) ([]models.GetExpertiseGroup, common.Pagination, *constants.ErrorResponse) {
	resultData := []models.GetExpertiseGroup{}
...
func (f expertiseGroupRepository) GetDetail(ctx context.Context, tx *sqlx.Tx, id string) (models.GetExpertiseGroup, *constants.ErrorResponse) {
	results := []models.GetExpertiseGroup{}
	...
	if err != nil {
		return models.GetExpertiseGroup{}, constants.ErrorInternalServer(err.Error())
	}
	if len(results) == 0 {
		return models.GetExpertiseGroup{}, utils.ErrDataNotFound("expertise group")
	}

	return results[0], nil
}

func (f expertiseGroupRepository) Create(ctx context.Context, tx *sqlx.Tx, data models.CreateExpertiseGroup) *constants.ErrorResponse {
...
func (f expertiseGroupRepository) Update(ctx context.Context, tx *sqlx.Tx, data models.UpdateExpertiseGroup) *constants.ErrorResponse {
	_, err := tx.ExecContext(
		ctx,
		updateQuery,
		data.Name,
...
func (f expertiseGroupRepository) Delete(ctx context.Context, tx *sqlx.Tx, id string) *constants.ErrorResponse {
	_, err := tx.ExecContext(
		ctx,
		deleteQuery,
		id,
